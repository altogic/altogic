...
async function verifyTypeScriptSetup(dir, intentDirs, typeCheckPreflight, config, cacheDir) {
    const tsConfigPath = _path.default.join(dir, config.typescript.tsconfigPath);
    try {
        var ref;
        // Check if the project uses TypeScript:
        const intent = await (0, _getTypeScriptIntent).getTypeScriptIntent(dir, intentDirs, config);
        if (!intent) {
            return {
                version: null
            };
